...
public class ReviewRestController {
    ...
    @PostMapping("/item")
    public ResponseEntity<?> addReviewItem(@Valid @RequestBody ReviewDto reviewDto, Errors errors) {
        ...
        for (Review r : reviews) {
            if (r.getItem().getName().equals(reviewDto.getItemName()) && r.getUser().getUsername().equals(review.getUser().getUsername())) {
                return ResponseEntity.badRequest().body(new ExceptionInfo("Вы уже оставляли отзыв на этот товар"));
            }
        }
        review.setShop(null);
    ...
    @PostMapping("/shop")
    public ResponseEntity<?> addReviewShop(@Valid @RequestBody ReviewDto reviewDto, Errors errors) {
        Review review = reviewMapper.dtoToReview(reviewDto);
        review.setDate(Date.valueOf(LocalDate.now()));
        ...
        List<Review> reviews = reviewService.getAll();
        for (Review r : reviews) {
            if (r.getShop().getName().equals(reviewDto.getShopName()) && r.getUser().getUsername().equals(review.getUser().getUsername())) {
                return ResponseEntity.badRequest().body(new ExceptionInfo("Вы уже оставляли отзыв на этот магазин"));
            }
        }
        review.setItem(null);
        reviewService.persist(review);
